...
export class HeaderComponent {
  ...
  constructor(
    ...
    this.user$ = this.login.getUserAsObservable().pipe(shareReplay());
    this.allowCreateNewUsers$ = this.data
      .getPreferences()
      .pipe(map(prefs => prefs.get('allowNewUsers')?.value === 'true'));
  }
        ...
        switchMap(dialog => {
          if (!dialog.cancelled) {
            ...
            return this.data
              ...
              .pipe(
                switchMap(user => this.login.attemptSignIn(username, password))
              );
          }
          return EMPTY;
        }),
